    ...
    <div class="w-full max-w-3xl">
        <div class="card">
            <div class="card-header">
                <h5 class="text-base font-semibold text-slate-900 dark:text-white">Edit: {{ $contact->name }}</h5>
                ...
                <form method="POST" action="{{ route('directory-contacts.update', $contact) }}">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="name" class="field-input @error('name') is-invalid @enderror" value="{{ old('name', $contact->name) }}" required>
                            @error('name')<p class="field-error">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="field-label">Email <span class="text-red-500">*</span></label>
                            <input type="email" name="email" class="field-input @error('email') is-invalid @enderror" value="{{ old('email', $contact->email) }}" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" class="field-input" value="{{ old('phone', $contact->phone) }}">
                        </div>
                        <div>
