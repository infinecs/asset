...
<a href="{{ route('outcome.report') }}" class="sidebar-link {{ request()->routeIs('outcome.report') ? 'active' : '' }} group relative" :class="{ 'justify-center': sidebarCollapsed && !mobileOpen }">
    <i class="bi bi-bar-chart"></i><span x-show="!sidebarCollapsed || mobileOpen">Task Log</span>
    <span x-show="sidebarCollapsed && !mobileOpen" x-cloak class="pointer-events-none absolute left-full ml-2 whitespace-nowrap rounded-md bg-slate-800 px-2 py-1 text-xs text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 z-50">Task Log</span>
</a>
