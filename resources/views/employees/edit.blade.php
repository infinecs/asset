...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('employees.update', $employee) }}" x-data="{ isManager: {{ old('is_manager', $employee->is_manager) ? 'true' : 'false' }}, status: '{{ old('status', $employee->status) }}' }">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div x-show="status === 'resigned'" x-cloak>
                    ...
                    <input type="date" name="resigned_date" class="field-input @error('resigned_date') is-invalid @enderror"
                           value="{{ old('resigned_date', $employee->resigned_date?->format('Y-m-d') ?? now()->toDateString()) }}" :disabled="status !== 'resigned'">
            ...
            @php
                $currentSubordinateIds = old('subordinate_ids', $employee->subordinates->pluck('id')->all());
                $currentManagedTeamIds = old('managed_team_ids', $employee->managedTeams->pluck('id')->all());
            @endphp
